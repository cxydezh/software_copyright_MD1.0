...
if (Test-Path "windows_deployment\web.config") {
    ...
    if ($duplicate_mime -gt 1) {
        ...
        # 添加正确的mimeMap
        $content = $content -replace '(<staticContent>)', '$1`n      <mimeMap fileExtension=".ison" mimeType="application/json" />'
...
try {
    ...
    $sites = Get-IISite | Where-Object {$_.Name -like "*software_copyright*"}
    if ($sites) {
