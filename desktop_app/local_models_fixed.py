...
class TemplateFolder:
    ...
    def add_file(self, file_name, file_path, programming_language=None,
                 ide_type=None, database_type=None, remarks=None):
        """添加模板文件"""
        cursor = self.db.execute_update(
    ...
    def get_all_files(self):
        """获取所有模板文件"""
        return self.db.execute_query("SELECT * FROM template_files ORDER BY created_time DESC")
    
    def get_recent_files(self, limit=10):
        """获取最近添加的文件"""
        return self.db.execute_query(
            "SELECT * FROM template_files ORDER BY created_time DESC LIMIT ?", 
            (limit,)
        )
    ...
    def get_file_by_id(self, file_id):
        """根据ID获取文件信息"""
        result = self.db.execute_query(
            "SELECT * FROM template_files WHERE id = ?", 
            (file_id,)
        )
        return result[0] if result else None
    ...
    def update_file(self, file_id, **kwargs):
        ...
        values = list(kwargs.values()) + [file_id]
        self.db.execute_update(
            f"UPDATE template_files SET {set_clause}, updated_time = ? WHERE id = ?",
            values + [datetime.now().isoformat()]
        )
    
    def delete_file(self, file_id):
        """删除文件记录"""
        self.db.execute_update("DELETE FROM template_files WHERE id = ?", (file_id,))
