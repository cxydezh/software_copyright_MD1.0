...
def update_permissions_table():
    ...
    try:
        ...
        print(f"[INFO] 备份现有权限记录: {len(existing_permissions)} 条")
        
        # 更新现有权限记录
        update_sql = """
            UPDATE permissions SET 
            can_confirm = %s, can_execute = %s, can_manage = %s, can_view_all = %s,
            can_edit_paper = %s, can_edit_patent = %s, is_expert = %s
            WHERE position = %s
        """
        
        # 权限配置
        permission_configs = {
            '普通业务员': (True, False, False, True, False, False, False),
            '项目执行者': (False, True, False, True, False, False, False),
            '论文编辑': (True, True, False, True, True, False, True),
            '专利编辑': (True, True, False, True, False, True, True),
            '专家': (True, True, True, True, True, True, True),
            '系统管理员': (True, True, True, True, True, True, True)
        }
        
        for position, config in permission_configs.items():
            cursor.execute(update_sql, config + (position,))
            print(f"[SUCCESS] 更新权限: {position}")
        ...
        # 检查哪些权限记录不存在
        cursor.execute("SELECT position FROM permissions")
        ...
        for perm_data in new_permissions:
            if perm_data[0] not in existing_positions:
                cursor.execute(insert_sql, perm_data)
                print(f"[SUCCESS] 插入新权限: {perm_data[0]}")
            else:
                print(f"[INFO] 权限已存在: {perm_data[0]}")
