...
from database.models import db, PaperProject, ProjectFile, Staff

api_paper_bp = Blueprint('api_paper', __name__)
...
@api_paper_bp.route('/paper/<int:paper_id>/upload', methods=['POST'])
@login_required
def upload_paper_file(paper_id):
    """上传论文项目文件"""
    try:
        # 检查项目是否存在
        ...
        # 生成相对路径用于数据库存储
        relative_path = os.path.join('uploads', 'papers', str(paper_id), unique_filename)
...
@api_paper_bp.route('/paper/<int:paper_id>/file/<int:file_id>/download')
@login_required
def download_paper_file(paper_id, file_id):
    """下载论文项目文件"""
    try:
        # 检查项目是否存在
        paper = PaperProject.query.get_or_404(paper_id)
        ...
        if not os.path.exists(file_path) or not os.path.isfile(file_path):
            # 文件不存在，自动删除数据库记录
            current_app.logger.warning(f"文件不存在，自动删除数据库记录 - 文件ID: {file_id}, 论文ID: {paper_id}, 文件路径: {file_path_stored}")
...
@api_paper_bp.route('/paper/<int:paper_id>/file/<int:file_id>/preview')
@login_required
def preview_paper_file(paper_id, file_id):
    """预览论文项目文件"""
    try:
        # 检查项目是否存在
        paper = PaperProject.query.get_or_404(paper_id)
...
@api_paper_bp.route('/paper/<int:paper_id>/file/<int:file_id>/delete', methods=['POST'])
@login_required
def delete_paper_file(paper_id, file_id):
    """删除论文项目文件"""
    try:
        # 检查项目是否存在
        paper = PaperProject.query.get_or_404(paper_id)
...
@api_paper_bp.route('/paper/<int:paper_id>/confirm', methods=['POST'])
@login_required
def confirm_paper_project(paper_id):
    """确认论文项目"""
    try:
        # 检查权限
...
@api_paper_bp.route('/paper/<int:paper_id>/assign_executor', methods=['POST'])
@login_required
def assign_paper_executor(paper_id):
    """分配执行者"""
    try:
        # 检查权限
        if not hasattr(current_user, 'user_type') or current_user.user_type != 'staff':
        ...
        if paper.status not in ['已确认', '进行中']:
            return jsonify({'success': False, 'message': '项目状态不正确'})
...
@api_paper_bp.route('/paper/<int:paper_id>/update_status', methods=['POST'])
@login_required
def update_paper_status(paper_id):
    ...
    try:
        ...
        now = datetime.utcnow()
        if new_status == '已投稿':
            paper.submit_time = now
        elif new_status == '已录用':
            paper.accept_time = now
        elif new_status == '已发表':
            paper.publish_time = now
...
@api_paper_bp.route('/paper/<int:paper_id>/delete_file/<int:file_id>', methods=['POST'])
@login_required
def delete_paper_file_by_id(paper_id, file_id):
    """删除项目文件"""
    try:
        # 检查项目是否存在
        paper = PaperProject.query.get_or_404(paper_id)
...
@api_paper_bp.route('/paper/staff_list', methods=['GET'])
@login_required
def get_paper_staff_list():
    """获取有论文编辑权限的员工列表"""
    try:
        # 检查权限
        if not hasattr(current_user, 'user_type') or current_user.user_type != 'staff':
        ...
        # 获取有论文编辑权限的员工
        staff_with_paper_permission = Staff.query.join(Permission, Staff.position_id == Permission.id).filter(
            Permission.can_edit_paper == True
        ).all()
        
        staff_data = []
        for staff in staff_with_paper_permission:
